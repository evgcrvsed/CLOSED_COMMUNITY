...
class DataBase:
    ...
    def get_all_users(self):
        result = self.get_cursor.execute("SELECT user_telegram_id FROM Data",).fetchall()
        result_array = [row[0] for row in result]
        return result_array

    def delete_user(self, user_telegram_id: int) -> None:
        self.get_cursor.execute('DELETE FROM Data WHERE user_telegram_id = ?;',(user_telegram_id,)).connection.commit()
